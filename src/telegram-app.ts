  ...
  private async prepareBlogArticle(ctx: Context): Promise<void> {
    ...
    try {
      ...
      const prompt = [
        ...
        "Обязательно используй навык make-valentin-article-package. До ImageGen проверь последние сюжеты через /Users/valentinbarko/.codex/skills/make-valentin-article-package/scripts/cover_rotation.py и выбери сцену, которая проходит ротацию по локации, действию, позе, одежде и кадрированию. Стол, рабочая поверхность или домашний кабинет допустимы только по смыслу статьи и не могут служить удобной зоной под заголовок. После визуальной проверки запиши фактический выбранный сюжет в историю ротации.",
        "",
